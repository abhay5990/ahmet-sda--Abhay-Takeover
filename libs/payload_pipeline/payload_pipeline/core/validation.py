...
def validate_resolved(
    ...
    *,
    ...
    Checks:
    1. ``item_id`` must not be empty (account listings only).
    2. ``price`` must be non-negative.
    3. In stock mode, ``credentials`` must not be empty (account listings only).
    """
    prefix = f"[{game}] " if game else ""
    # Item listings (SAB, New World) use dedicated resolved models that do not
    # inherit from ResolvedAccountBase — skip the account-specific checks.
    if category == "item":
        if hasattr(subject, "price") and subject.price < 0:
            raise SourceValidationError(f"{prefix}Resolved item has negative price: {subject.price}")
        return
