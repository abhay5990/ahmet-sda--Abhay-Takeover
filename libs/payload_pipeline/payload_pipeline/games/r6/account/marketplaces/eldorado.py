...
class R6EldoradoBuilder(BaseEldoradoBuilder):
    ...
    def build_payload(
        ...
        return self.build_base_payload(
            ...
            attributes={
                _ATTR_RANK: account.current_rank_attr or self._resolve_rank(account),
                _ATTR_BLACK_ICE: self._resolve_black_ice(account.black_ice_count),
                "rainbow-six-game-purchased": account.game_purchased_attr or ("purchased-yes" if account.has_game else "purchased-no"),
                "rainbow-six-operators": self._resolve_operators(account.operator_count),
                "rainbow-six-previous-rank": account.previous_rank_attr or self._resolve_previous_rank(account),
                "rainbow-six-ranked-unlocked": account.ranked_unlocked_attr or ("ranked-unlocked-yes" if account.ranked_ready else "ranked-unlocked-no"),
