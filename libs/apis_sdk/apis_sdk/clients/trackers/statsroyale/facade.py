"""
StatsRoyale high-level facade.

Provides a clean consumer-facing API that coordinates:
- Optional proxy selection
- Delegation to the low-level client

The player profile endpoint is unauthenticated and read-only.
Arena mapping and data transformation are app-level concerns,
not part of this SDK facade.
"""
...
class StatsRoyaleFacade:
    """
    High-level StatsRoyale tracker interface.

    Coordinates proxy selection around the low-level StatsRoyaleClient.
    ...
    def get_profile(
        ...
        *,
        ...
        return self._client.get_profile(player_tag, proxy_url=proxy_url)
