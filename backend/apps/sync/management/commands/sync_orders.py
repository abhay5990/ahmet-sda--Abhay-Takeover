...
class Command(BaseCommand):
    help = 'Sync orders/products from a provider into the local database.'

    def add_arguments(self, parser):
        parser.add_argument(
            'account',
            type=str,
            help='IntegrationAccount slug (e.g. "eldorado-store4gamers")',
        )
        parser.add_argument(
            '--mode',
            type=str,
        ...
        parser.add_argument(
            ...
            help='Resource type to sync (default: orders)',
        )
        parser.add_argument(
            '--dry-run',
            action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        if detail_failed:
            self.stdout.write(self.style.WARNING(
                f"Detail fetch failures: {detail_failed} orders skipped."
            ))
