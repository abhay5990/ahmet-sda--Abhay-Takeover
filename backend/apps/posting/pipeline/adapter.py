...
def _build_ct_bridge_publisher(fallback_publisher=None):
    ...
    try:
        ...
        url = getattr(settings, 'CT_BRIDGE_URL', '') or ''
        secret = getattr(settings, 'CT_BRIDGE_SECRET', '') or ''
...
def prepare(
    *,
    ...
    Args:
        ...
        ct_bridge_source_id:    Source item ID for CT bridge (e.g. LZT item_id).
        ct_bridge_game:         Game slug for CT bridge (e.g. 'fortnite').
        ct_bridge_eldorado_store: Eldorado store slug for CT bridge (e.g. 'ezsmurfmart').
    ...
    request = build_request(
        ...
        ct_bridge_eldorado_store=ct_bridge_eldorado_store,
        category=category,
    )
