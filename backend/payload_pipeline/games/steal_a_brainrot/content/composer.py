...
class SabItemComposer:
    ...
    def _build_title(self, s):
        ...
        if s.item_name: parts.append(s.item_name)
        if s.rarity: parts.append(f"[{s.rarity.capitalize()}]")
        if s.ms_min > 0:
    ...
    def _build_description(self, s):
        ...
        if s.item_name: lines.append(f"Item: {s.item_name}")
        if s.rarity:
            emoji = RARITY_EMOJI.get(s.rarity.lower(), "")
        ...
        if s.mutations: lines.append(f"Mutations: {', '.join(s.mutations)}")
