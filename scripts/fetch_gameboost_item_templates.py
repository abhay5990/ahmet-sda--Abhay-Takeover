"""Fetch Gameboost item-offer templates for all item games.

Reads the API key from the DB (gameboost-store4gamers),
hits /v2/item-offers/templates/{slug} for each game,
and saves the raw results to tmp/gameboost/items/{slug}.json.

Usage (from project root):
    python scripts/fetch_gameboost_item_templates.py
"""
...
# ── Config ──────────────────────────────────────────────────────────
OUTPUT_DIR = os.path.join(PROJECT_ROOT, 'tmp', 'gameboost', 'items')
BASE_URL = 'https://api.gameboost.com/v2/item-offers/templates'
...
for g in games:
    ...
    if 'items' in cats:
        item_slugs.append(g['slug'])

print(f'Found {len(item_slugs)} item games')
...
for i, slug in enumerate(item_slugs, 1):
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            success += 1
            print(f'  [{i}/{len(item_slugs)}] OK: {slug}')
        else:
            failed.append((slug, resp.status_code))
            print(f'  [{i}/{len(item_slugs)}] FAIL ({resp.status_code}): {slug}')
    except Exception as e:
        failed.append((slug, str(e)))
        print(f'  [{i}/{len(item_slugs)}] ERROR: {slug} — {e}')
